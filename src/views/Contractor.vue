...
<div id="main-box">
            ...
            <div class="data-grid">
                <table>
                    ...
                    <tbody>
                        <tr v-for="contrator in contractors" v-bind:key="contrator.id" v-on:click="activeRow(contrator.id, $event)">
                                <td>{{contrator.id}}</td>
                                <td>{{contrator.name}}</td>
                                <td>{{contrator.adress}}</td>
                                <td>{{contrator.nip}}</td>
                                <td>{{contrator.regon}}</td>
                        </tr>
                        <tr class="blank">
                                <td></td>
                                <td></td>
...
export default {
  ...
  mounted:function(){
        this.getContractor() //method1 will execute at pageload
  },
  data(){
        return{
            invoiceExist: false,
    ...
    methods:{
        getContractor() {
            axios.get(`http://localhost:3128/companies`).then(res => {
                this.contractors = res.data.companies
            });
        },
        activeRow( id, event ){
        ...
        deleteInvoices() {
            let invoiceId = this.active_row.pop()
